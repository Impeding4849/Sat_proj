...
try:
    ...
    from tensorflow.keras.models import Model # For create_qml_model
    from tensorflow.keras.layers import Dense, Dropout, BatchNormalization, Activation # For create_qml_model
...
# Base column names (mirroring quantum_model.py and baseline_optimizer.py)
FEATURE_COLS_BASE = [
    'altitude_plane_{}_km',
...
# TARGET_COLS_ML from quantum_model.py are ['mean_revisit_time_hr', 'mean_achieved_quality']
...
def get_qml_model_and_scalers():
    """
    Loads a pre-trained Quantum-Hybrid Keras model (weights), its associated scalers,
    and the hyperparameters used to define its architecture (both QML-specific and NN-classical).
    """
    model_weights_filename = 'quantum_model.weights.h5'
    feature_scaler_filename = 'feature_scaler_qml.joblib'
    target_scaler_filename = 'target_scaler_qml.joblib'
    best_qml_only_hps_filename = 'best_qml_only_hps.joblib' # QML part HPs (n_qubits, n_qlayers)
    best_nn_hps_filename = 'best_nn_hps.joblib'         # Classical part HPs (from neural_net_model.py)

    if not TF_KERAS_AVAILABLE:
        print("TensorFlow/Keras not imported. Cannot load Keras model.")
        return None, None, None
    if not PENNYLANE_AVAILABLE:
        print("PennyLane not imported. Cannot load QML model components.")
        return None, None, None

    try:
        # 1. Load QML-specific hyperparameters (n_qubits, n_qlayers)
        qml_hps = joblib.load(best_qml_only_hps_filename)
        print(f"Successfully loaded QML-only hyperparameters from: {best_qml_only_hps_filename}")

        # 2. Load Neural Network hyperparameters (for classical part)
        nn_hps = joblib.load(best_nn_hps_filename)
        print(f"Successfully loaded NN hyperparameters from: {best_nn_hps_filename}")

        # 3. Recreate the model architecture using these HPs
        input_dim = (N_TOTAL_PLANES * N_FEATURES_PER_PLANE) + 1
        output_dim = 2 
        
        model = create_qml_model_from_hps(input_dim, output_dim, qml_hps, nn_hps)
        ...
        print(f"Successfully recreated Quantum-Hybrid Keras model using combined HPs and loaded weights from: {model_weights_filename}")
        ...
        print(f"Error: One or more files not found: '{model_weights_filename}', "
              f"'{feature_scaler_filename}', '{target_scaler_filename}', "
              f"'{best_qml_only_hps_filename}', or '{best_nn_hps_filename}'.")
        print("Please run 'neural_net_model.py' and then 'quantum_model.py' first to generate these files.")
        return None, None, None
    except Exception as e:
        print(f"An error occurred while loading the Quantum-Hybrid Keras model or scaler: {e}")
        return None, None, None
    return model, feature_scaler, target_scaler


# This function reconstructs the QML model using saved QML-specific HPs and NN HPs.
def create_qml_model_from_hps(input_dim_classical, num_outputs_final, qml_hps_values, nn_hps_values):
    """
    Reconstructs the QML model architecture using the best QML-specific hyperparameters
    (n_qubits, n_qlayers) and fixed classical hyperparameters from the NN model.
    """
    if not TF_KERAS_AVAILABLE or not PENNYLANE_AVAILABLE:
        raise ImportError("TensorFlow/Keras or PennyLane not available for model reconstruction.")

    n_qubits_val = qml_hps_values['n_qubits']
    n_qlayers_val = qml_hps_values['n_qlayers']

    dev_dynamic = qml.device("default.qubit", wires=n_qubits_val)

    @qml.qnode(dev_dynamic, interface='tf', diff_method='backprop')
    def quantum_circuit_reconstruct(inputs, weights):
        qml.AngleEmbedding(inputs, wires=range(n_qubits_val))
        qml.StronglyEntanglingLayers(weights, wires=range(n_qubits_val))
        return [qml.expval(qml.PauliZ(i)) for i in range(n_qubits_val)]

    weight_shapes = {"weights": (n_qlayers_val, n_qubits_val, 3)}
    qlayer = qml.qnn.KerasLayer(quantum_circuit_reconstruct, weight_shapes, output_dim=n_qubits_val)

    inputs_classical = tf.keras.Input(shape=(input_dim_classical,))

    # --- Classical Pre-processing Layers (using loaded NN HPs) ---
    # Key names must match those saved by neural_net_model.py in best_nn_hps.joblib
    x = Dense(units=nn_hps_values['units_layer_1'])(inputs_classical)
    x = BatchNormalization()(x)
    x = Activation(nn_hps_values['activation_1'])(x)
    x = Dropout(nn_hps_values['dropout_1'])(x)

    x = Dense(units=nn_hps_values['units_layer_2'])(x)
    x = BatchNormalization()(x)
    x = Activation(nn_hps_values['activation_2'])(x)
    x = Dropout(nn_hps_values['dropout_2'])(x)
    
    x = Dense(units=nn_hps_values['units_layer_3'])(x)
    x = BatchNormalization()(x)
    x = Activation(nn_hps_values['activation_3'])(x)

    x_quantum_prep = Dense(n_qubits_val)(x)
    ...
    # --- Classical Post-processing Layer (using NN HPs, e.g., from layer 3) ---
    y = Dense(units=nn_hps_values['units_layer_3'])(quantum_outputs)
    y = BatchNormalization()(y)
    y = Activation(nn_hps_values['activation_3'])(y)

    outputs_final = Dense(num_outputs_final, activation='linear')(y)
    model = Model(inputs=inputs_classical, outputs=outputs_final)

    model.compile(optimizer=tf.keras.optimizers.AdamW(learning_rate=nn_hps_values['learning_rate']),
...
class ConstellationProblem(Problem): # Changed from ElementwiseProblem
    """
    Defines the optimization problem using the Quantum-Hybrid model.
    """
...
def run_optimization(n_planes, model, feature_scaler, target_scaler):
    """
    Runs the NSGA-II optimization for a specific number of planes using the QML model.
    """
    print(f"\n--- Optimizing for {n_planes} Orbital Plane(s) using Quantum-Hybrid Model ---")
...
def create_interactive_plot(all_results, best_n_planes, best_solution_idx):
    ...
    fig.add_trace(go.Scatter3d(
        ...
        name='Overall Best Solution (QML Model)', hoverinfo='none'
    ))

    fig.update_layout(
        title_text='<b>Pareto Fronts: QML Model Constellation Optimization</b><br><sup>Cost vs. Revisit Time vs. Quality</sup>',
...
def save_results_to_csv(all_results_dict, filename="qml_optimizer_pareto_fronts.csv"):
    """
...
def main():
    """
    Main function to run the optimization using the Quantum-Hybrid Keras model.
    """
    if not TF_KERAS_AVAILABLE:
        print("\nAborting execution due to TensorFlow/Keras library not being available.")
        return
    if not PENNYLANE_AVAILABLE:
        print("\nAborting execution due to PennyLane library not being available.")
        return

    model, feature_scaler, target_scaler = get_qml_model_and_scalers()
    if model is None or feature_scaler is None or target_scaler is None:
        print("Quantum-Hybrid Keras Model and/or scalers could not be loaded. Aborting optimization.")
        return
    ...
    print("\n\n======================================================================")
    print(f" Final Recommended Design ({best_n_planes}-Plane Constellation) - QML Model ")
    print("======================================================================")
    ...
    save_results_to_csv(all_results, filename="qml_optimizer_pareto_fronts.csv")
